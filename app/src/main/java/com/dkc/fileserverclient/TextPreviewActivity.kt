...
class TextPreviewActivity : AppCompatActivity() {
    ...
    private val serviceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            audiobookService?.paginationCallback = paginationCallback
        ...
        override fun onServiceDisconnected(name: ComponentName?) {
            audiobookService?.paginationCallback = null
            audiobookService = null
        }
    }

    private val paginationCallback = object : TextPaginationManager.Callback {
        override fun onPageContentChanged(content: CharSequence, state: TextPaginationManager.PageState) {
            runOnUiThread { textContentTextView.text = content; updateProgressBar(state) }
        }
        override fun onError(message: String) { runOnUiThread { showErrorState(message) } }
        override fun onLoading(loading: Boolean, message: String?) {
            runOnUiThread { if (loading) showLoadingState(message) else showContentState() }
        }
        override fun onChaptersReady(chapters: List<TextPaginationManager.ChapterInfo>) {
            runOnUiThread { showChapterList(chapters); statusLabel.isVisible = false }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val serviceIntent = Intent(this, AudiobookService::class.java).apply {
        ...
        rootLayout.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                if (!isLayoutReady && textContentTextView.height > 0) {
                    ...
                    isLayoutReady = true
                    if (audiobookService != null) updateServiceDisplayParams()
                }
            }
        })
    }
    ...
    private fun updateProgressBar(state: TextPaginationManager.PageState) {
    ...
    private fun setupGestureDetector() {
        gestureDetector = GestureDetector(this, object : GestureDetector.SimpleOnGestureListener() {
            override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
                ...
                val x = e.x; val sw = resources.displayMetrics.widthPixels
    ...
    private fun manualNextPage() { if (isAutoPlay) { isAutoPlay = false; audiobookService?.stopAutoPlay() }; audiobookService?.nextPage() }
    private fun manualPrevPage() { if (isAutoPlay) { isAutoPlay = false; audiobookService?.stopAutoPlay() }; audiobookService?.previousPage() }

    private fun showVoiceMenu() {
        PopupMenu(this, voiceMenuButton).apply {
            menu.add("开始自动朗读"); menu.add("停止朗读"); menu.add("选择TTS引擎")
            setOnMenuItemClickListener {
                when (it.title) {
                    "开始自动朗读" -> { isAutoPlay = true; audiobookService?.startAutoPlay() }
                    "停止朗读" -> { isAutoPlay = false; audiobookService?.stopAutoPlay() }
    ...
    private fun showEngineSelectionDialog() {
        ...
        val engines = tts.engines.toList(); tts.shutdown()
        if (engines.isEmpty()) { Toast.makeText(this, "没有可用引擎", Toast.LENGTH_SHORT).show(); return }
        ...
        AlertDialog.Builder(this).setTitle("选择引擎")
            .setSingleChoiceItems(names, if (checked >= 0) checked else -1) { d, i ->
                ...
                audiobookService?.reinitializeTts(engines[i].name); d.dismiss()
            }.setNegativeButton("取消", null).show()
    }

    private fun showChapterDialog() {
        statusLabel.isVisible = true; statusLabel.text = "正在加载章节..."
        audiobookService?.loadChapters()
    }

    private fun showChapterList(chapters: List<TextPaginationManager.ChapterInfo>) {
        val sorted = chapters.sortedBy { it.startCharOffset }
        val offset = audiobookService?.getCurrentPageState()?.absoluteCharOffset ?: 0
        ...
        listView.adapter = object : ArrayAdapter<TextPaginationManager.ChapterInfo>(this, android.R.layout.simple_list_item_1, sorted) {
        ...
        AlertDialog.Builder(this).setView(view).setNegativeButton("取消", null).create().also {
            listView.setOnItemClickListener { _, _, p, _ -> audiobookService?.jumpToChapter(sorted[p]); it.dismiss() }
            it.show(); if (idx >= 0) listView.setSelection(idx)
        }
    }
    ...
    private fun showAppearanceDialog() {
        ...
        val origFont = currentFontSize; val origBg = currentBackgroundColor
        val origSpeed = prefs.getFloat("tts_speed", 1.0f)

        seekFont.max = 18; seekFont.progress = (origFont - 12).toInt()
        ...
        seekFont.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar?, p: Int, fromUser: Boolean) {
                fontSizeVal.text = "${p + 12}sp"; textContentTextView.textSize = (p + 12).toFloat()
            }
        ...
        seekSpeed.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar?, p: Int, fromUser: Boolean) {
                ...
                speedVal.text = String.format("%.1fx", speed); audiobookService?.setSpeechRate(speed)
            }
            override fun onStartTrackingTouch(sb: SeekBar?) {}
            override fun onStopTrackingTouch(sb: SeekBar?) {}
        })
        ...
        btnC.setOnClickListener { currentBackgroundColor = Color.parseColor("#FAF0D7"); rootLayout.setBackgroundColor(currentBackgroundColor) }
        btnG.setOnClickListener { currentBackgroundColor = Color.parseColor("#C8E6C9"); rootLayout.setBackgroundColor(currentBackgroundColor) }
        ...
        cancelBtn.setOnClickListener {
            ...
            applyAppearance(); updateServiceDisplayParams(); dialog.dismiss()
        }
        applyBtn.setOnClickListener {
            currentFontSize = (seekFont.progress + 12).toFloat()
            val speed = 0.5f + seekSpeed.progress / 100f * 1.5f
            saveAppearancePrefs(); prefs.edit().putFloat("tts_speed", speed).apply()
            audiobookService?.setSpeechRate(speed)
            applyAppearance(); textContentTextView.post { updateServiceDisplayParams() }
            dialog.dismiss()
        }
        dialog.show()
    }
    ...
    private fun saveAppearancePrefs() { prefs.edit().putFloat("font_size", currentFontSize).putInt("bg_color", currentBackgroundColor).apply() }
    ...
    private fun getContrastColor(bg: Int) = if ((Color.red(bg)*0.299 + Color.green(bg)*0.587 + Color.blue(bg)*0.114) > 127.5) Color.DKGRAY else Color.argb(0x80, 0xFF, 0xFF, 0xFF)
    private fun enableImmersiveMode() {
        window.decorView.systemUiVisibility = View.SYSTEM_UI_FLAG_FULLSCREEN or View.SYSTEM_UI_FLAG_HIDE_NAVIGATION or View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY
    }
    private fun showLoadingState(msg: String?) { loadingProgress.isVisible = true; textContentTextView.isVisible = false; errorTextView.isVisible = false; statusLabel.isVisible = true; statusLabel.text = msg ?: "加载中..." }
    private fun showContentState() { loadingProgress.isVisible = false; textContentTextView.isVisible = true; errorTextView.isVisible = false; statusLabel.isVisible = false }
    private fun showErrorState(msg: String) { loadingProgress.isVisible = false; textContentTextView.isVisible = false; errorTextView.isVisible = true; errorTextView.text = msg; statusLabel.isVisible = false }
}